package com.anwesh.uiprojects.squaretoxview

/**
 * Created by anweshmishra on 23/06/19.
 */
...
class SquareToXView(ctx : Context) : View(ctx) {
    ...
    data class STXNode(var i : Int, val state : State = State()) {

        private var next : STXNode? = null
        private var prev : STXNode? = null
        ...
        fun draw(canvas : Canvas, paint : Paint) {
            canvas.drawSTXNode(i, state.scale, paint)
        ...
        fun getNext(dir : Int, cb : () -> Unit) : STXNode {
            var curr : STXNode? = prev
            if (dir == 1) {
                curr = next
            }
            if (curr != null) {
    ...
    data class SquareToX(var i : Int) {

        private val root : STXNode = STXNode(0)
        private var curr : STXNode = root
